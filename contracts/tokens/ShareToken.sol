...
contract ShareToken is MintableToken, SharesInterface {
    ...
    function getName() view external returns (string) { return name; }
    function getSymbol() view external returns (string) { return symbol; }
    function getDecimals() view external returns (uint) { return decimals; }
    function getCreationTime() view external returns (uint) { return creationTime; }
    function toSmallestShareUnit(uint quantity) view external returns (uint) { return SafeMath.mul(quantity, uint256(10) ** decimals); }
    function toWholeShareUnit(uint quantity) view external returns (uint) { return quantity / (uint256(10) ** decimals); }
}
