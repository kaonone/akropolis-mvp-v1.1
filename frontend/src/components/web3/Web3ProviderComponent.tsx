...
import { fetchAccountAction, fetchNetworkAction } from "../../redux/actions/web3Action";
...
const HALF_MINUTE = ONE_SECOND * 30;
...
class Web3ProviderComponent extends React.Component<PropsFromDispatch, {}> {
    ...
    private initPoll() {
        if (!interval) {
            interval = setInterval(this.props.fetchAccount, HALF_MINUTE);
        }
    }

    private initNetworkPoll() {
        if (!networkInterval) {
            networkInterval = setInterval(this.props.fetchNetwork, HALF_MINUTE);
        }
    }
}
